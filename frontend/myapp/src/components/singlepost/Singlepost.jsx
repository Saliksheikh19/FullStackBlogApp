...
export default function SinglePost() {
  ...
  useEffect(() => {
  const getPost = async()=>{
    const res =await axios.get("http://localhost:8000/posts/" + path)
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`http://localhost:8000/posts/${singlePost._id}`, {
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`http://localhost:8000/posts/${singlePost._id}`, {
